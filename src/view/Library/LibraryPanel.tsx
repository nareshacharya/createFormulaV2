...
import { PegaService, Ingredient, Formula, IngredientAttribute } from '../../services/pega';
import { evaluateQuery } from '../../utils/queryEvaluator';
import { FilterGroup } from '../../components/QueryBuilder';
...
const LibraryPanel = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [ingredientsData, formulasData, attributesData] = await Promise.all([
  ...
  useEffect(() => {
    const handleFormulaSelectionsUpdated = (data: { count: number; selectedIds: string[] }) => {
  ...
  useEffect(() => {
    const handleWorkAreaIngredientsUpdate = (data: { ingredients: string[] }) => {
  ...
  const syntheticCount = ingredients.filter(i => i.type === 'synthetic').length;
    ...
    <div className="h-full flex flex-col bg-white border-r border-gray-200">
      ...
      <div className="p-3 border-t border-gray-200 bg-gray-50 flex-shrink-0">
        <div className="text-xs text-gray-500 text-center">
          {activeTab === 'ingredients' && `${filteredIngredients.length} of ${ingredients.length} ingredients`}
          {activeTab === 'formulas' && `${filteredFormulas.length} of ${formulas.length} formulas`}
          {activeTab === 'attributes' && `${filteredAttributes.length} of ${attributes.length} attributes`}
        </div>
      </div>
