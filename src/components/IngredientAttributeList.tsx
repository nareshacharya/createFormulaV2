...
const IngredientAttributeList = ({ 
  ...
  const filteredAttributes = attributes.filter(attribute => {
    ...
    if (appliedFilters.category && appliedFilters.category !== attribute.category.toLowerCase().replace(' ', '-')) {
      matchesAdvancedFilters = false;
    }
    
    if (appliedFilters.required !== undefined && appliedFilters.required !== attribute.isRequired) {
      matchesAdvancedFilters = false;
    }
    ...
    <div className="space-y-0">
      {filteredAttributes.map((attribute) => {
        ...
        return (
          <ListRow
            key={attribute.id}
            onHover={(isHovered) => setHoveredRow(isHovered ? attribute.id : null)}
            onClick={() => {
              if (!isDisabled) {
