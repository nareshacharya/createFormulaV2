...
const SuppliersSection = ({ ingredient }: SuppliersSectionProps) => {
  // Mock suppliers data
  const mockSuppliers = [
    {
      id: 'SUP001',
            ...
            <div key={supplier.id} className="border border-gray-200 rounded-lg p-4">
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <Badge variant={getQualityVariant(supplier.quality)} size="sm">
                      {supplier.quality}
                    </Badge>
                  </div>
                  <p className="text-sm text-gray-600">Supplier ID: {supplier.id}</p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-semibold text-gray-900">
                    ${supplier.price.toFixed(2)}
                  </p>
                  <p className="text-sm text-gray-600">per {supplier.unit}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">{supplier.minOrder}</p>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">
                    Last Order
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">{supplier.contact.name}</p>
                </div>
              </div>
              ...
              <div className="flex space-x-2 mt-4">
                <button className="flex-1 px-3 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors cursor-pointer">
                  <i className="ri-shopping-cart-line mr-2"></i>
                  Request Quote
                </button>
                <button className="px-3 py-2 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-200 rounded-md hover:bg-gray-100 transition-colors cursor-pointer">
      ...
      <div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div>
              <p className="text-sm text-blue-800 font-medium">Performance Metrics</p>
              <p className="text-sm text-blue-700 mt-1">
                Detailed supplier performance analytics including delivery times, quality scores, 
                and pricing trends would be available through integration with procurement systems.
              </p>
            </div>
          </div>
        </div>
