...
const DocumentsSection = ({ ingredient }: DocumentsSectionProps) => {
  // Mock documents data
  const mockDocuments = [
    {
      id: 'DOC001',
    ...
    <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3 flex-1">
                  ...
                  <div className="flex-shrink-0 w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                    <i className={`${getFileIcon(doc.format)} text-blue-600`}></i>
                  </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {['Safety', 'Quality', 'Compliance', 'Technical', 'Certification'].map((category) => {
            const count = mockDocuments.filter(doc => doc.type === category).length;
            return (
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-start">
          ...
          <div>
            <p className="text-sm text-yellow-800 font-medium">Document Expiration Notice</p>
            <p className="text-sm text-yellow-700 mt-1">
              1 document is expiring soon. Please review and update certifications as needed.
            </p>
          </div>
        </div>
      </div>
