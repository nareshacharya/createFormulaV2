...
const QueryBuilder = ({ onQueryChange, onApply, onClear }: QueryBuilderProps) => {
  ...
  const fields = [
    ...
    { name: 'type', label: 'Type', type: 'select', options: ['natural', 'synthetic', 'nature_identical'] },
    { name: 'category', label: 'Category', type: 'select', options: ['Essential Oils', 'Aroma Chemicals', 'base'] },
    { name: 'supplier', label: 'Supplier', type: 'select', options: ['Givaudan', 'Firmenich', 'IFF', 'Symrise'] },
    { name: 'status', label: 'Status', type: 'select', options: ['active', 'inactive', 'palette', 'analytical', 'sers_review'] },
    { name: 'mac', label: 'MAC', type: 'number' },
    { name: 'odorProfile', label: 'Odor Profile', type: 'text' },
    { name: 'volatility', label: 'Volatility', type: 'select', options: ['top', 'heart', 'base'] },
    { name: 'allergens', label: 'Allergens', type: 'multiselect', options: ['Limonene', 'Linalool', 'Citronellol', 'Geraniol'] }
  ];
  ...
  const addRuleToGroup = (groupId: string) => {
    ...
    const addToRules = (rules: (FilterRule | FilterGroup)[]): (FilterRule | FilterGroup)[] => {
  ...
  const removeRule = (ruleId: string) => {
    const removeFromRules = (rules: (FilterRule | FilterGroup)[]): (FilterRule | FilterGroup)[] => {
  ...
  const updateRule = (ruleId: string, updates: Partial<FilterRule>) => {
    const updateInRules = (rules: (FilterRule | FilterGroup)[]): (FilterRule | FilterGroup)[] => {
      return rules.map(rule => {
  ...
  const renderRule = (rule: FilterRule | FilterGroup, index: number, parentRules: (FilterRule | FilterGroup)[], parentCombinator: 'and' | 'or', parentGroupId: string) => {
    if ('rules' in rule) {
      // This is a group
      return (
        <div key={rule.id} className="border border-gray-300 rounded p-3 bg-gray-50 ml-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-2">
              <select
                value={rule.combinator}
                onChange={(e) => updateGroupCombinator(rule.id, e.target.value as 'and' | 'or')}
          ...
          <div className="space-y-2">
            ...
            {rule.rules.map((subRule, subIndex) => 
              renderRule(subRule, subIndex, rule.rules, rule.combinator, rule.id)
            )}
          </div>
        </div>
      );
